...
`identity/` is deliberately **not** a tool: no `workflow, no data/, deliberately.** Don't fit it into the tool layout. Its internal law is `identity/scg/SCG_MARK_CONSTITUTION.md` — read it before touching anything under `identity/`.
