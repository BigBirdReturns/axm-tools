...
(async()=>{
 ...
 try{
  ...
  const page=await ctx.newPage();listen(page);await page.goto(service.url,{waitUntil:'networkidle'});await page.waitForFunction(()=>document.querySelector('#connection-status')?.textContent.startsWith('CONNECTED ·'));
  ...
  check('zero held records do not create an error message',(await page.locator('#connection-error').innerText()).trim()==='');
  check('unconfigured runtime is not displayed as an outage',!(await page.locator('#runtime-summary').innerText()).includes('unavailable'));
  await page.click('[data-publication="0"]');await page.waitForSelector('#qualified-save');
  check('synthetic state survives both interfaces',/synthetic integration record/i.test(await page.locator('#modal-body').innerText()));
  check('primary-cell cost displayed from instrument',!(await page.locator('#modal-body .v').first().innerText()).includes('—'));
  const rec=store.read('records',store.list('records')[0]),before=JSON.stringify(rec);await page.getByText('Retained identity and verification',{exact:true}).click();check('exact record identity visible', (await page.locator('#modal-body').innerText()).includes(rec.sha256));
