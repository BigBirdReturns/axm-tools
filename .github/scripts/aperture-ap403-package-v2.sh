...
work="${RUNNER_TEMP:-/tmp}/aperture-ap403-v2-package"
out="$work/out"
archive="$out/axm-aperture-ap403-second-screen-target-v2.tar.gz"
rebuilt="$work/rebuilt.tar.gz"
cold="$work/cold"
rm -rf "$work"
mkdir -p "$work/stage" "$out" "$cold"

bash .github/scripts/aperture-ap403-qualify-v2.sh "$source_root" | tee "$out/qualification.log"
...
bash .github/scripts/aperture-ap403-qualify-v2.sh "$cold/aperture-target" | tee "$out/cold-reconstruction.log"
...
cp "$source_root/receipts/AP-403.json" "$out/"
...
const receipt = {
  ...
  format: 'axm-aperture-ap403-source-carrier-receipt/2',
...
(
  cd "$out"
  sha256sum \
    axm-aperture-ap403-second-screen-target-v2.tar.gz \
