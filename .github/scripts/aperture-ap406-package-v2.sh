...
work="${RUNNER_TEMP:-/tmp}/aperture-ap406-v2-package"
out="$work/out"
archive="$out/axm-aperture-ap406-spoiler-controls-target-v2.tar.gz"
rebuilt="$work/rebuilt.tar.gz"
cold="$work/cold"
rm -rf "$work"
mkdir -p "$work/stage" "$out" "$cold"

bash "$repo_root/.github/scripts/aperture-ap406-qualify-v2.sh" "$source_root" | tee "$out/qualification.log"
...
bash "$repo_root/.github/scripts/aperture-ap406-qualify-v2.sh" "$cold/aperture-target" | tee "$out/cold-reconstruction.log"
...
cp "$source_root/receipts/AP-406.json" "$out/"
...
const receipt = {
  ...
  canonical_ap406_accepted: false,
  ...
  format: 'axm-aperture-ap406-source-carrier-receipt/2',
  ...
  tests_passed_per_execution: 122,
  transaction: 'AP-406',
};
...
(
  cd "$out"
  sha256sum \
    axm-aperture-ap406-spoiler-controls-target-v2.tar.gz \
