...
source_root="aperture-carrier/source/aperture-target"
work="${RUNNER_TEMP:-/tmp}/aperture-ap404-package"
out="$work/out"
archive="$out/axm-aperture-ap404-query-transactions-target-v1.tar.gz"
rebuilt="$work/rebuilt.tar.gz"
cold="$work/cold"
rm -rf "$work"
mkdir -p "$work/stage" "$out" "$cold"

bash .github/scripts/aperture-ap404-qualify.sh "$source_root" | tee "$out/qualification.log"
...
bash .github/scripts/aperture-ap404-qualify.sh "$cold/aperture-target" | tee "$out/cold-reconstruction.log"

cp aperture-carrier/SOURCE_ARCHIVE.sha256 "$out/"
cp aperture-carrier/TARGET_SHA256SUMS "$out/"
cp "$source_root/SOURCE_MANIFEST.sha256" "$out/"
cp "$source_root/PREDECESSOR_RECEIPT.json" "$out/"
cp "$source_root/receipts/AP-404.json" "$out/"
...
const receipt = {
  ...
  canonical_ap404_accepted: false,
  canonical_g3_accepted: false,
  carrier_authority: 'transport_and_qualification_only',
  format: 'axm-aperture-ap404-source-carrier-receipt/1',
...
(
  cd "$out"
  sha256sum \
    axm-aperture-ap404-query-transactions-target-v1.tar.gz \
