...
OUTPUT = Path(os.environ.get("MW_WORKING_MODEL_LIVE_MANIFEST", "/tmp/manzanita-working-model-v1.1-live-byte-readback.json"))
...
def fetch(url: str) -> tuple[bytes, str]:
    request = urllib.request.Request(url, headers={
        ...
        "User-Agent": "axm-tools-manzanita-working-model-live-readback/1.1",
    })
    ...
    for attempt in range(1, 121):
        ...
        for relative in FILES:
            ...
            try:
                ...
                row = {"url": url, "bytes": len(data), "sha256": digest(data), "content_type": content_type}
        ...
        last = {"attempt": attempt, "mismatches": mismatches, "root_directory_link": root_ok, "root_error": root_error}
        if not mismatches and root_ok:
            ...
            if f'content="{RELEASE}"' not in index_text or f"const RELEASE = '{RELEASE}';" not in app_text:
                raise AssertionError("local release identity differs")
            payload = {
                "schema": "manzanita-works/working-model-live-byte-readback@3",
                ...
                "files": {relative: {"expected": expected[relative], "observed": observed[relative]} for relative in FILES},
        ...
        time.sleep(5)
    raise AssertionError(json.dumps(last, indent=2))
