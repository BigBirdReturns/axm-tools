...
def main() -> None:
    ...
    registrations = {}
    for logical_key, template_receipt in template.pop("registrationTemplates").items():
        receipt = json.loads(json.dumps(template_receipt))
        receipt["asset_id"] = assets[logical_key]["asset_id"]
        receipt["asset_key"] = logical_key
        receipt["receipt_sha256"] = canonical_sha(receipt)
        registrations[f"{receipt['asset_id']}:access"] = receipt
    ...
    index = (args.source_root / "template/index.html").read_text(encoding="utf-8").replace("__MANZANITA_DATA__", compact)
    (args.output / "index.html").write_text(index, encoding="utf-8")
    shutil.copy2(args.source_root / "template/app.js", args.output / "app.js")
    shutil.copy2(args.source_root / "template/style.css", args.output / "style.css")
    ...
    receipt = {
        "schema": "manzanita-works/semantic-access-review-build@2",
