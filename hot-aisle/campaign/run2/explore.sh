#!/usr/bin/env bash
# Run 2 - POST-HOC EXPLORATION (see EXPLORE.md). Forces ROCM_AITER_FA; one repeat; not the registered result.
...
PORT=8000; OUT=/tmp/run2-explore; HF_CACHE="${HF_CACHE:-$HOME/hf-cache}"
SERVE_FLAGS=(--max-model-len 16384 --max-num-seqs 256 --gpu-memory-utilization 0.90 --no-enable-prefix-caching --attention-backend ROCM_AITER_FA)
...
serve() {
  ...
  printf '{"schema":"second-run/arm-environment@1","run":"run2-explore-posthoc","recorded_at":"%s","kind":"%s","host":"%s","gpu":"%s","driver_or_rocm":"%s","image":"%s","vllm_version":"%s","model":"%s","revision":"%s","serve_flags":"%s","env":"%s","kernel":"%s"}\n' \
...
cell() {  # cell <shape> <in> <out> <conc> <n> <rep>
  ...
  dk exec vllm vllm bench serve --backend openai --base-url "http://127.0.0.1:$PORT" --model "$MODEL" \
    ...
    --metadata run=run2-explore-posthoc shape="$shape" model_revision=$REVISION precision=FP8-dynamic workload_id="run2-$shape-v1" cache_policy=no-prefix-cache cell="$shape-c$c-r$r" \
    > "$OUT/${f%.json}.log" 2>&1 || { log "cell $f FAILED"; rm -f "$OUT/$f"; }
}

bench() {
  local t0=$SECONDS r row
  local plan=( "long 8192 512 1 16" "long 8192 512 8 64" "long 8192 512 32 128" 
