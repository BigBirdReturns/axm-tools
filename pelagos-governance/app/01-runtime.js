'use strict';
const RELEASE = 'DDV-PEL-002/0.2.0';
const WORKSPACE_SCHEMA = 'ddv/pelagos-governance-workspace@2';
...
function metrics(){const open=PUBLIC.exceptions.filter(exceptionOpen);const critical=open.filter(x=>x.Severity==='Critical').length;const high=open.filter(x=>x.Severity==='High').length;const held=PUBLIC.claims.filter(x=>['Critical','High'].includes(x.Risk)).length;return [
  [critical,'critical decisions'],[high,'high decisions'],[held,'high-risk claims'],[PUBLIC.instruments.length,'public instruments'],[workspace.source_receipts.length,'local sources'],[workspace.receipts.length,'local receipts']
].map(([n,l])=>`<div class="metric"><strong>${n}</strong><span>${esc(l)}</span></div>`).join('')}
function bestAction(){const open=PUBLIC.exceptions.filter(exceptionOpen).sort((a,b)=>(SEV[b.Severity]||0)-(SEV[a.Severity]||0));return open[0]||null}
function renderToday(){const best=bestAction();const secondary=PUBLIC.founder_decisions.filter(d=>!decisionFor(d.Exception.split(';')[0])).slice(0,3);const held=['send email','schedule meeting','sign instrument','publish claim','move money','release controlled data'];return viewHead('Today','One best next action, the safe secondary moves, and the outside effects still withheld.')+
`<div class="metric-grid">${metrics()}</div><div class="grid two" style="margin-top:14px"><section class="card paper hero-action"><div class="card-body"><div class="kicker">Best next action</div>${best?`<h3>${esc(best['Decision Required'])}</h3><p class="muted">${esc(best['Mechanism / Failure'])}</p><div>${severityTag(best.Severity)} <span class="tag">${esc(best.Domain)}</span> <span class="tag">default ${esc(best.Default)}</span></div><p><b>Owner:</b> ${esc(best['Proposed Owner'])}</p><button class="btn primary" data-open-decision="${esc(best['Exception ID'])}">Open bounded decision</button>`:'<h3>No unresolved public exceptions</h3><p>The public baseline still remains a candidate until an authorized Pelagos record says otherwise.</p>'}</div></section>
...
<div class="grid two" style="margin-top:14px"><section class="card"><div class="card-head"><h3>State separations that keep Pelagos honest</h3></div><div class="card-body">${PUBLIC.invariants.slice(0,10).map(x=>`<div class="receipt"><b>${esc(x.Rule)}</b><br><span class="muted">${esc(x['Failure Mode'])}</span></div>`).join('')}</div></section><section class="card"><div class="card-head"><h3>External-effect firewall</h3></div><div class="card-body"><div class="callout stop">This runtime has zero adapters for email, calendar, payment, publication, signature, procurement, or source-system writeback.</div><div class="record-list">${held.map(x=>`<div class="record"><span class="tag hold">held</span> ${esc(x)}</div>`).join('')}</div></div></section></div>`}
