...
Sources blocking automation use the explicit `--observed manifest.json` route: `{ "source-id": { "path": "capture.html", "sha256": "exact-file-digest", "url": "registered-url", "observed_at": "actual-capture-time" } }`. Captures must stay inside the manifest directory and preserve their original timestamp. Missing, stale, mismatched and inaccessible observations remain visible. The initial 23-of-25 capture is preserved under history/. Official SGLang and SkyPilot documentation redirects were subsequently reviewed, their canonical URLs explicitly registered and a new capture made. The collector still refuses unreviewed redirects. See history/source-moves.json.
