# AXM Aperture .NET SDK 8.0.423 toolchain ferry
...
The workflow resolves SDK `8.0.423` from Microsoft's .NET 8 release metadata, selects the unique `linux-x64` SDK archive, verifies the downloaded archive against the SHA-512 value published in that metadata, extracts it, requires `dotnet --version` to equal `8.0.423`, compiles and runs an original C# smoke program, and emits a deterministic repacked SDK plus complete checksums and a machine receipt.
