...
| **Essential Attention** | `…/axm-tools/essential-attention/` | **Cream Editorial operating cockpit inside Dark Ecosystem custody** | paper `#F0EBE0` inside void `#0D0C09` | **olive `#7C7F57` custody + amber case emphasis; ember only for blocks** | The administrative case file is a governed dossier with enterprise operating habits, not a KPI dashboard. Exact SCG mark, case topology, role-aware home, attention assistant, guided path, docket rows, evidence wells, activity timeline, and mono receipts carry the estate identity while the cartridge remains white-label. |
